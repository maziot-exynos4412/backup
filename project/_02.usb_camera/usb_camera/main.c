...
void save_bmp(char *src)
{
        ...
        // Set BITMAPINFOHEADER 设置 BMP 信息头
        ...
        */
        ...
        for(i = 0; i < 480; i++)
        fwrite((unsigned char *)((unsigned int)src + 640 * 3 * (479 - i)), 640 * 3, 1, fp);
...
// 初始化 frambuufer
static int lcd_init(unsigned char *fbname)
{
        /* 打开帧缓冲文件 */
        fb = open(fbname, 2);
        if(fb < 0)
...
//显示一行数据
void lcd_draw_line(unsigned char *src, unsigned char* dst, unsigned int width)
{
        unsigned char r, g, b;
                ...
                unsigned int image_width, unsigned int image_height)
{
        ...
        {
                lcd_draw_line(src, dst, image_width);
                src += (image_width * 3);
                dst += line_width;
        }
        return 0;
...
static int camera_init(void)
{
        ...
        if(tV4L2Cap.capabilities & V4L2_CAP_STREAMING)
        {
                ...
                for(i = 0; i < tV4l2ReqBuffs.count; i++)
                {
                        ...
                        //映射空间地址
                        pucVideBuf[i] = mmap(0 /* start anywhere */, tV4l2Buf.length, PROT_READ, MAP_SHARED, iFd,
                                        tV4l2Buf.m.offset);
        ...
        while(1)
        {
                ...
                if(data == 4)
                {
                        ...
                        for(i = 0; i < 640; i++)
                        {
                                for(j = 0; j < 480; j++)
                                {
                                        if(i < 15 || i > 625 || j < 15 || j > 465)
                                        lcd_draw_point(i, j, 0xff0000);
                                }
                        }
